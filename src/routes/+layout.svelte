...
<header class="sticky top-0">
	<div class="flex items-center px-5 py-6 space-x-7 backdrop-blur-xl bg-slate-50/50 dark:bg-slate-950/50 dark:border-b dark:border-slate-50">
		...
		<button
			class="sm:hidden cursor-pointer ml-auto text-black dark:text-white"
			onclick={toggleMenu}
			aria-label="Toggle menu"
			aria-expanded={menuOpen}
		>
            {#if menuOpen}
                <svg xmlns="http://www.w3.org/2000/svg" class="w-7 h-7 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
